...
If you want to try it get it from the Play Store or directly from GitHub https://github.com/greenaddress/abcore/releases/tag/v0.46alphaPoC
...
The dependencies are fetched from either a Debian or Arch mirror (Arch providing 0.12.1 while Debian providing 0.11.2) and the node is meant to be used as a personal node when on the go (either by using a mobile wallet that allows to connect to a remote and specific node or even directly on your mobile device).
